...
class IRCClientThread(threading.Thread):
    ...
    def _irc_366(self, nick, params, prefix):
        if params:
            self.emit("status", text=f"End of NAMES for {params[1] if len(params) > 1 else params[0]}")
...
class IRCMainWindow(QMainWindow):
    ...
    def _autocomplete_nick(self) -> bool:
        ...
        match = next((nick for nick in candidates if nick.lower().startswith(token.lower())), None)
        if match is None:
            return False

        new_text = text[:start] + match + text[pos:]
        self.txtInput.blockSignals(True)
        try:
            self.txtInput.setPlainText(new_text)
            try:
                new_cursor = self.txtInput.textCursor()
                new_cursor.setPosition(start + len(match))
    ...
    def poll_events(self):
        while True:
            ...
            if kind == "status":
                ...
                users = ev.get("users", [])
                self.channels_users[channel] = set(users)
                self._ensure_channel(channel)
                if channel == self.current_target:
                    self._refresh_user_list(sorted(users, key=str.lower))
            elif kind == "join":
                channel = ev.get("channel", "")
                nick = ev.get("nick", "")
                ...
                self._append_html_line(f'<div>{_html_span("ts", ev.get("ts", _ts()))} {html.escape(ev.get("old", ""))} is now known as {html.escape(ev.get("new", ""))}</div>')
            elif kind == "notice":
